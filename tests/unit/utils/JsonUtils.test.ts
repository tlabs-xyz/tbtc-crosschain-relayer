...
describe('JsonUtils', () => {
  ...
  describe('getAllJsonOperations and getAllJsonOperationsByStatus', () => {
    ...
    test('should get operations by status', async () => {
      // Create test deposits with different statuses
      const queuedDeposit1 = createTestDeposit({
        status: DepositStatus.QUEUED,
      });
      const queuedDeposit2 = createTestDeposit({
        status: DepositStatus.QUEUED,
      });
      ...
      const initializedFilePath = path.join(
        TEST_DIR,
        `${initializedDeposit.id}.json`
      );
      ...
      const queuedOperations = await getAllJsonOperationsByStatus(
        DepositStatus.QUEUED
      );
      const initializedOperations = await getAllJsonOperationsByStatus(
        DepositStatus.INITIALIZED
      );
      const finalizedOperations = await getAllJsonOperationsByStatus(

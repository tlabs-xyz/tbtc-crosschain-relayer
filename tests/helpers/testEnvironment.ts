 ...
 * Handles test database, services, and parallel test execution with PostgreSQL
 */
 ...
 * Uses PostgreSQL with unique schemas for test isolation
 */
export class TestDatabaseManager {
    ...
    if (this.prisma) {
      ...
      if (this.testSchema) {
        try {
          ...
          await this.prisma.$executeRaw`DROP SCHEMA IF EXISTS ${this.testSchema} CASCADE`;
        } catch (_error) {
   ...
   * Creates a unique PostgreSQL schema for test isolation
   */
  private setupDatabaseUrl(): string {
    ...
    // Create a unique schema for this test run to ensure test isolation
 ...
 * Uses separate PostgreSQL schemas for test isolation
 */

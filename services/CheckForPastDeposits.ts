...
export const checkForPastDeposits = async ({
  pastTimeInMinutes,
  latestBlock,
}: {
  ...
  try {
    ...
    // Process events
    LogMessage(
      `Found ${events.length} DepositInitialized events in the last ${pastTimeInMinutes} minutes`
    );

    for (const event of events) {
      const { fundingTx, reveal, l2DepositOwner, l2Sender } = event.args as any;
      LogMessage(
        `Processing missed DepositInitialized event | Tx: ${fundingTx}`
      );
      ...
      if (!existingDeposit) {
        const newDeposit: Deposit = createDeposit(
          fundingTx,
          reveal,
          l2DepositOwner,
          l2Sender
        ...
        LogMessage(
          `Missed deposit found and initializing | Id: ${newDeposit.id}`
        );
    ...
    LogMessage(
      `Error checking for missed initializeDeposit transactions: ${error}`
    );
  }

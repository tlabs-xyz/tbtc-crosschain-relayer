...
export class StarknetChainHandler extends BaseChainHandler<StarknetChainConfig> {
  constructor(config: StarknetChainConfig) {
    ...
    if (config.chainType !== CHAIN_TYPE.STARKNET) {
      throw new Error(`Incorrect chain type ${config.chainType} provided to StarknetChainHandler.`);
    }
  }

  protected async initializeL2(): Promise<void> {
    logger.info(`Initializing StarkNet L2 components for ${this.config.chainName}`);
  ...
  protected async setupL2Listeners(): Promise<void> {
    if (!this.config.useEndpoint) {
      logger.warn(`StarkNet L2 Listener setup NOT YET IMPLEMENTED for ${this.config.chainName}.`);
      ...
      logger.info(`StarkNet L2 Listeners skipped for ${this.config.chainName} (using Endpoint).`);
    }
  }

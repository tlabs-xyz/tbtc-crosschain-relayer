...
export class SuiChainHandler extends BaseChainHandler {
  ...
  protected async setupL2Listeners(): Promise<void> {
    ...
    try {
      LogMessage(`Setting up Sui L2 event subscription for ${this.config.chainName}`);
      ...
      LogMessage(`Sui L2 DepositInitialized event subscription setup for ${this.config.chainName}`);
  ...
  private async handleDepositInitializedEvent(event: any): Promise<void> {
    try {
      LogMessage(`Received DepositInitialized event on ${this.config.chainName}`);
      ...
      const deposit = { ...depositData, chain: this.config.chainName } as unknown as Deposit;
  ...
  async submitVaaToSui(depositId: string, vaaBytes: string): Promise<boolean> {
    if (!this.suiClient || !this.suiKeypair) {
      LogError(`Cannot submit VAA to Sui: Missing client or keypair`, new Error('Missing client or keypair'));
      return false;
    }
    
    if (!this.bitcoinDepositorPackageId || !this.receiverStateId || !this.gatewayStateId ||
        !this.gatewayCapabilitiesId || !this.treasuryId || !this.wormholeStateId ||
        !this.tokenBridgeStateId || !this.tbtcTokenStateId) {
      LogError(`Cannot submit VAA to Sui: Missing required object IDs`, new Error('Missing required object IDs'));
      return false;
    }
    ...
    try {
      ...
      LogMessage(`VAA submission transaction for ${depositId} executed: ${signedTx.digest}`);
      ...
      if (status === TxStatus.SUCCESS) {
        ...
        LogError(`VAA submission for deposit ${depositId} failed: ${error}`, new Error(error || 'Unknown error'));
        return false;
      }
      
    } catch (error: any) {
      LogError(`Error submitting VAA to Sui for ${depositId}: ${error.message}`, error);
  ...
  private addPendingVaaSubmission(depositId: string, vaaBytes: string): void {
    ...
    const existing = this.pendingVaaSubmissions.find(p => p.depositId === depositId);
    if (existing) {
      // Update existing record
      existing.lastAttempt = Date.now();
      existing.attempts += 1;
      LogMessage(`Updated pending VAA submission for ${depositId}: attempt ${existing.attempts}`);
    } else {
  ...
  private async processPendingVaaSubmissions(): Promise<void> {
    ...
    LogMessage(`Processing ${this.pendingVaaSubmissions.length} pending VAA submissions`);
    
    const now = Date.now();
    const submissionsToProcess = this.pendingVaaSubmissions.filter(
      p => now - p.lastAttempt > this.SUBMISSION_RETRY_DELAY_MS && p.attempts < this.MAX_SUBMISSION_ATTEMPTS
    );
    ...
    for (const submission of submissionsToProcess) {
      LogMessage(`Retrying VAA submission for ${submission.depositId} (attempt ${submission.attempts + 1})`);
      
      const success = await this.submitVaaToSui(submission.depositId, submission.vaaBytes);
      ...
      if (success) {
        ...
        LogMessage(`Successfully processed VAA submission for ${submission.depositId} on retry`);
      }
    }
    
    // Clean up submissions that have exceeded max attempts
    const expired = this.pendingVaaSubmissions.filter(p => p.attempts >= this.MAX_SUBMISSION_ATTEMPTS);
    if (expired.length > 0) {
      LogWarning(`${expired.length} VAA submissions exceeded max retry attempts and will be dropped`);
      this.pendingVaaSubmissions = this.pendingVaaSubmissions.filter(
      ...
      for (const submission of expired) {
        LogError(`VAA submission for ${submission.depositId} failed after ${submission.attempts} attempts`, new Error('Max retry attempts exceeded'));
      }
    }
  }
  ...
  async checkForPastDeposits(options: {
    ...
    try {
      ...
      LogMessage(`Found ${pastEvents.data.length} past DepositInitialized events`);

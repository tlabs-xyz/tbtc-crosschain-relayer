...
// Wormhole VAA fetch retry configuration (matching SuiChainHandler timing)
  ...
  implements ChainHandlerInterface
{
   ...
   * Returns the base64-encoded VAA string, or null if not available.
   */
  private async fetchVAAFromAPI(sequence: string): Promise<string | null> {
    try {
      const emitterChain = this.config.network === NETWORK.MAINNET ? '2' : '10002';
  ...
  public async bridgeEvmDeposit(deposit: Deposit): Promise<void> {
    ...
    try {
      ...
      const vaaBase64 = await this.fetchVAAFromAPI(deposit.wormholeInfo.transferSequence);

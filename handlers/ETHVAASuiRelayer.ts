...
export class ETHVAASuiRelayer {
  ...
  async startListening(): Promise<void> {
    try {
      ...
      this.tokenBridgeContract.on(
        ...
        async (
          ...
          try {
            ...
            LogError(`Error processing TokensTransferredWithPayload event: ${error.message}`, error);
          }
        }
      );
      ...
      LogError(`Error starting ETHVAASuiRelayer listener: ${error.message}`, error);
      throw error;
    }
  }
  ...
  async checkForPastTransfers(options: {
    ...
    try {
      LogMessage(`Checking for past token transfers from block ${options.fromBlock} to ${options.toBlock}`);
      ...
      for (const event of events) {
        ...
        const [token, amount, recipientChain, recipient, nonce, payload, fee] = args;
        ...
        LogMessage(`Processing past token transfer event with VAA ID: ${vaaId}`);
        await this.fetchAndRelayVAA(vaaId, transactionHash);
      }
    } catch (error: any) {
      LogError(`Error checking for past token transfers: ${error.message}`, error);
    }
  }
   ...
   */
  private async fetchAndRelayVAA(vaaId: string, depositId: string): Promise<void> {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Guardian API returned error: ${response.status} ${response.statusText}`);
      }
      ...
      if (success) {
        LogMessage(`Successfully submitted VAA ${vaaId} to Sui for deposit ${depositId}`);
        this.processedVAAs.add(vaaId);
      } else {
        LogWarning(`Failed to submit VAA ${vaaId} to Sui for deposit ${depositId}, will retry later`);
        this.scheduleVAAReFetch(vaaId, depositId);
      }
    } catch (error: any) {
      LogError(`Error fetching/relaying VAA ${vaaId} for deposit ${depositId}: ${error.message}`, error);
   ...
   */
  private scheduleVAAReFetch(vaaId: string, depositId: string, retryCount: number = 0): void {
    ...
    if (retryCount >= MAX_RETRIES) {
      LogWarning(`Exceeded maximum retries (${MAX_RETRIES}) for VAA ${vaaId}, giving up`);
      return;
    }
    
    const delay = RETRY_DELAYS[Math.min(retryCount, RETRY_DELAYS.length - 1)];
    
    LogMessage(`Scheduling retry #${retryCount + 1} for VAA ${vaaId} in ${delay / 1000} seconds`);
    
    setTimeout(() => {
      this.fetchAndRelayVAA(vaaId, depositId)
        .catch((error) => {
  ...
  private async getCurrentTransactionHash(): Promise<string | null> {
    try {
      ...
      LogError(`Error getting current transaction hash: ${error.message}`, error);
      return null;
    }
  }
} 